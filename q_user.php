 ...
 if($rowid>0){
    if($query = mysqli_query($connect,"SELECT * FROM q_user WHERE rowid = '".$rowid."'")){
      ...
      if(isset($_REQUEST["submit"])){
        ...
        if($query = mysqli_query($connect,"UPDATE q_user SET name = '".$name."', email = '".$email."', phone = '".$phone."', address = '".$address."', fk_sport = '".$fkSport."', status = '".$status."' ".$sql." WHERE rowid = '".$rowid."'")){
    ...
    if($query = mysqli_query($connect,"SELECT * FROM q_user WHERE rowid = '".$rowid."'")){
      ...
      if(isset($_REQUEST["submit"])){
        ...
        $status   = $_REQUEST['status'];
        $sql = "";
        if($password!=''){ $password=$password;}else{$password=MD5('123456');}
        if($query = mysqli_query($connect,"INSERT INTO q_user (name, email, phone, address, fk_sport, status, password) VALUES ( '".$name."', '".$email."', '".$phone."', '".$address."', '".$fkSport."', '".$status."' , '".$password."')")){
          $errors = []; // Store errors here
                  ...
                  <div class="box-body">
                    ...
                    <div class="box">
                      ...
                      <div class="box-body pad">
                          <textarea class="textarea" name="address" required="required" placeholder="Direcci&oacute;n" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"><?=$array['address'];?></textarea>
                      </div>
                    </div>
